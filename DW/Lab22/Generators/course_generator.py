...
def create_courses_sql(coursesT1, coursesT2,T1,T2, is_New):
    if is_New:
        start_parameter = 1
    ...
    with open('dane_kursow.sql', 'a', encoding='utf-8') as file:
        for i in range(start_parameter, end_parameter):
            ...
            creation_date = fake.date_between(start_date=startDate, end_date=endDate).strftime('%d-%m-%Y')
            data = (
                i,
                course['nazwa'],
                creation_date,
                fake.date_between(start_date=datetime.strptime(creation_date, '%d-%m-%Y'), end_date=updateEndDate).strftime(
                    '%d-%m-%Y'),
                category,
                random.choice([0, 1]),
                random.choice([0, 1]),
