...
def generate_user_course_data(coursesT1, coursesT2, usersT1, usersT2, T1,T2, is_New):
    ...
    pattern = r"INSERT INTO Kurs \(ID, Nazwa, Data_utworzenia, Kategoria, Czy_posiada_forum, Czy_posiada_testy, Czy_posiada_konsultacje, Szacowana_dlugosc_kursu, Semestr\)\n\s+VALUES \((\d+), '[^']+', '(\d{4}-\d{2}-\d{2})', '[^']+', \d, \d, \d, \d+, \d\);"
    ...
    with open('dane_użytkownik_kurs.sql', file_mode, encoding='utf-8') as file:
        ...
        for i in range(users_start_parameter, users_end_parameter):
            ...
            for match in matches:
                ...
                if int(kurs_id) == i:
                    join_date = fake.date_between(start_date=datetime.strptime(data_utworzenia, '%Y-%m-%d'), end_date='today').strftime('%Y-%m-%d')
                    ...
                    data = (
                        ...
                        join_date,
                        fake.date_between(start_date=datetime.strptime(join_date, '%Y-%m-%d'), end_date='today').strftime(
                            '%Y-%m-%d'),
                        random.choice([0, 1]),
                        random.randint(1, 50),
